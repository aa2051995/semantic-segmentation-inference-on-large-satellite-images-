...
def slice_image(
    ...
    # read image
    image_pil = gdal.Open(image) # to do  image_pil = read_image_as_pil(image)
    verboselog("image.shape: " + str(image_pil.RasterXSize)) # to do verboselog("image.shape: " + str(image_pil.size))

    image_width, image_height = image_pil.RasterXSize,image_pil.RasterYSize # to do image_width, image_height = image_pil.size
    if not (image_width != 0 and image_height != 0):
        raise RuntimeError(f"invalid image size: {image_pil.RasterYSize} for 'slice_image'.") # raise RuntimeError(f"invalid image size: {image_pil.size} for 'slice_image'.")
    ...
    #image_pil_arr = np.asarray(image_pil) #to do
    # iterate over slices
    for slice_bbox in slice_bboxes:
        ...
        brx = slice_bbox[2]
        bry = slice_bbox[3]
        image_pil_slice = "" # image_pil_arr[tly:bry, tlx:brx]  #to doooo
        ...
        if out_ext:
            ...
            try:
                suffix = Path(image).suffix # to dooo
        ...
        sliced_image = SlicedImage(
            image=image_pil_slice, coco_image=coco_image, starting_pixel=[slice_bbox[0], slice_bbox[1],slice_bbox[2],slice_bbox[3]]
        )
