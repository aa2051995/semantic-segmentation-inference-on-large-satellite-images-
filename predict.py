...
def get_prediction(
    ...
    # read image as pil
    image_as_pil = image #image_as_pil = read_image_as_pil(image)
    # get prediction
...
def get_sliced_prediction(
    ...
    for group_ind in range(num_group):
        ...
        for image_ind in range(num_batch):
            tile = slice_image_result.starting_pixels[group_ind * num_batch + image_ind]
            minx = tile[0]
            miny = tile[1]
            maxx = tile[2]
            maxy = tile[3]
            ...
            #data1 = band1.ReadAsArray(i1, j1, new_cols, new_rows)
            #data2 = band2.ReadAsArray(i1, j1, new_cols, new_rows)
            #data3 = band3.ReadAsArray(i1, j1, new_cols, new_rows)
            data1 = band1.ReadAsArray(minx, miny, maxx-minx, maxy-miny)
            data2 = band2.ReadAsArray(minx, miny, maxx-minx, maxy-miny)
            data3 = band3.ReadAsArray(minx, miny, maxx-minx, maxy-miny)
            img_1 = np.dstack([data1,data2,data3])
            #print("slice shape",img_1.shape)
            image_list.append(img_1)#image_list.append(slice_image_result.images[group_ind * num_batch + image_ind]) # to dooooooo
    ...
    print(" num of nodes before preocessing",len(object_prediction_list))
    # merge matching predictions
    if len(object_prediction_list) > 1:
        object_prediction_list = postprocess(object_prediction_list)
    print("num of nodes after preocessing",len(object_prediction_list))
    ...
    if num_slices <= 48:
        ...
        img_1 = np.dstack([data1,data2,data3])
        return PredictionResult(
            image=img_1, object_prediction_list=object_prediction_list, durations_in_seconds=durations_in_seconds,final=True
        )
    else:
        return PredictionResult(
            image=image, object_prediction_list=object_prediction_list, durations_in_seconds=durations_in_seconds,final=True
        )
